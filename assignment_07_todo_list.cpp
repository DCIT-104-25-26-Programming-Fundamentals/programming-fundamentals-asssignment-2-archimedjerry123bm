...
int main() {
    ...
    do {
        ...
        // Input validation to ensure the user entered an integer
        if (cin.fail()) {
...
// Deletes a task selected by 1-based index
void deleteTask(vector<string>& tasks) {
    if (tasks.empty()) {
    ...
    if (cin.fail()) {
        cin.clear();
        cin.ignore(numeric_limits<streamsize>::max(), '\n');
        cout << "Invalid input! Task number must be an integer.\n\n";
        return;
    }

    // Convert 1-based menu choice to 0-based vector index
